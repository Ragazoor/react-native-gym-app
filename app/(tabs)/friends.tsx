...
export default function FriendsWorkoutsScreen() {
  const [expandedFriend, setExpandedFriend] = useState<number | undefined>(undefined);
  ...
  return (
    <>
      {friendsData && friendsData.length > 0 ? <FlatList
        data={friendsData}
        keyExtractor={(item) => item.id.toString()}
        renderItem={renderFriend}
        contentContainerStyle={styles.container}
      /> :
        <ThemedText style={styles.noWorkoutsText}>No booked workouts</ThemedText>}
    </>
  );
};
